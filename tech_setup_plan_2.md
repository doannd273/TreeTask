...
Bản tài liệu này đóng vai trò Architecture Setup Guideline tập trung vào chất lượng code, môi trường build và pipeline phân phối. Đã chốt cấu trúc dựa trên nhu cầu triển khai thực tế của Hệ thống.
...
- **Nhiệm vụ**:
  - Checkout code & Setup JDK 17 (hoặc 21 tùy build).
...
* **Detekt (`io.gitlab.arturbosch.detekt`)**:
  ...
  - Cấu hình qua file `.detekt.yml`. Những Rules nghiêm ngặt hơn ktlint, đảm bảo Clean Code tuyệt đối.

* **Git Hooks (Pre-commit) - NGĂN CHẶN MÃ RÁC LÊN GITHUB**: 
  - Dự án sẽ tích hợp một script gài vào `.git/hooks/pre-commit` tự động chạy build lệnh kiểm tra lint ngay trên máy tính của Developer. 
  - **Luồng hoạt động**: Mỗi lần Dev gõ `git commit`, script trigger. Nếu code dính lỗi detekt hoặc sai format, quá trình commit sẽ **bị chặn đứng lập tức**. Việc ép chuẩn hóa diễn ra tức tế tại local, không làm tốn tài nguyên CI kiểm tra.
...
| Flavor | Build Type | Mục đích | API Base Url | Cấu hình thêm |
|---|---|---|---|---|
| **`dev`** | **`debug`** | Dev code hằng ngày. Do test trên **Thiết bị thật**, API sẽ trỏ thẳng **IP nội bộ (LAN) của máy chủ NodeJS** (VD: `http://192.168.1.xxxx:5000`) | Cấu hình IP động ở `buildConfigField`. Kích hoạt LeakCanary & Chucker. Log level Verbose.
