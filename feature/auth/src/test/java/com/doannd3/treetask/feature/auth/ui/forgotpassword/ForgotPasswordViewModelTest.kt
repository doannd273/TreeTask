...
class ForgotPasswordViewModelTest {
    ...
    @Test
    fun `submit email success moves to reset input and emits success effect`() =
        runTest {
            //  GIVEN
    ...
    @Test
    fun `resend otp clears otp and calls forgot password flow`() =
        runTest {
            val email = "doan@test.com"

            coEvery { forgotPasswordUseCase(email) } returns
                ApiResult.Success(
    ...
    @Test
    fun `submit reset password success emits reset password success effect`() =
        runTest {
            val email = "doan@test.com"
            val otp = "123456"
    ...
    @Test
    fun `submit email while loading does not call forgot password twice`() =
        runTest {
            val email = "doan@test.com"
